...
class Banker: Operation {
    ...
    init(bankerNumber: Int, client: Client?, notification: NSNotification.Name, headOffice: HeadOffice, lockForBusinessTime: NSLock) {
        self.bankerNumber = bankerNumber
        self.client = client
        self.notification = notification
        self.headOffice = headOffice
        self.lockForBusinessTime = lockForBusinessTime
    }
    ...
    override func main() {
        if let client = self.client {
            ...
            print("\(client.waitingNumber)번 \(clientGrade) \(client.taskType)업무 완료 \(self.businessTime)") // 대출을 실행하면 대출 업무가 끝납니다
        }
    ...
    func updateBusinessTime(time: Float) {
        lockForBusinessTime.lock()
        self.businessTime += time
        lockForBusinessTime.unlock()
