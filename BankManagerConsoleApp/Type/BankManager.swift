...
class BankManager {
    ...
    private func createBanker(numberOfBanker: Int) {
        for i in 1...numberOfBanker {
            ...
            let banker = Banker(bankerNumber: i ,client: nil, notification: notification, headOffice: self.headOffice)
    ...
    @objc func updateBankCounter(notification: Notification) {
        ...
        if clientQueue.isNotEmpty {
            ...
            let banker = Banker(bankerNumber: bankerNumber, client: clientQueue.removeFirst(), notification: notificationNumber, headOffice: self.headOffice)
            operationQueue.addOperation(banker)
        }
        lock.unlock()
    }
